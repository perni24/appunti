# Indice Linux

## Percorso Base

### Fondamenti del sistema

- [[Linux/Pagine/Architettura Linux kernel shell e user space|Architettura Linux: kernel, shell e user space]]
- [[Linux/Pagine/Distribuzioni Linux|Distribuzioni Linux]]
- [[Linux/Pagine/Filesystem Hierarchy Standard|Filesystem Hierarchy Standard]]
- [[Linux/Pagine/Terminale e comandi fondamentali|Terminale e comandi fondamentali]]
- [[Linux/Pagine/Variabili ambiente e PATH|Variabili d'ambiente e PATH]]
- [[Linux/Pagine/Manuali man info e help|Manuali e documentazione: man, info e help]]

### File, utenti e permessi

- [[Linux/Pagine/File directory e link|File, directory e link]]
- [[Linux/Pagine/Gestione utenti|Gestione utenti, gruppi e permessi]]
- [[Linux/Pagine/Permessi chmod chown e umask|Permessi chmod, chown e umask]]
- [[Linux/Pagine/sudo e privilegi amministrativi|sudo e privilegi amministrativi]]
- [[Linux/Pagine/ACL e permessi avanzati|ACL e permessi avanzati]]

### Shell

- [[Linux/Pagine/Bash|Bash]]
- [[Linux/Pagine/Pipe e ridirezioni|Pipe e ridirezioni]]
- [[Linux/Pagine/Espansioni della shell|Espansioni della shell]]
- [[Linux/Pagine/Variabili e quoting|Variabili e quoting]]
- [[Linux/Pagine/Job control|Job control]]
- [[Linux/Pagine/Elaborazione testi con grep sed e awk|Elaborazione testi con grep, sed e awk]]
- [[Linux/Pagine/Espressioni regolari|Espressioni regolari]]

## Percorso Intermedio

### Processi e servizi

- [[Linux/Pagine/Processi PID e segnali|Processi, PID e segnali]]
- [[Linux/Pagine/Job control|Job control e processi in background]]
- [[Linux/Pagine/Monitoraggio delle risorse|Monitoraggio delle risorse]]
- [[Linux/Pagine/systemd|systemd]]
- [[Linux/Pagine/Unit e dipendenze systemd|Unit e dipendenze systemd]]
- [[Linux/Pagine/Timer systemd|Timer systemd]]
- [[Linux/Pagine/Cron e Anacron|Cron e Anacron]]
- [[Linux/Pagine/Logging con journalctl|Logging con journalctl]]

### Gestione software

- [[Linux/Pagine/Repository e package manager|Repository e package manager]]
- [[Linux/Pagine/apt dnf e pacman|apt, dnf e pacman]]
- [[Linux/Pagine/Dipendenze e librerie condivise|Dipendenze e librerie condivise]]
- [[Linux/Pagine/Compilazione da sorgente|Compilazione da sorgente]]
- [[Linux/Pagine/Gestione pacchetti universali|Gestione pacchetti universali: Flatpak, Snap e AppImage]]
- [[Linux/Pagine/Installazione Linguaggi User-space|Installazione linguaggi in user-space]]

### Storage e filesystem

- [[Linux/Pagine/Partizionamento dei dischi|Partizionamento dei dischi]]
- [[Linux/Pagine/Mount e umount|Mount e umount]]
- [[Linux/Pagine/Configurazione di fstab|Configurazione di fstab]]
- [[Linux/Pagine/Filesystem ext4 Btrfs e XFS|Filesystem ext4, Btrfs e XFS]]
- [[Linux/Pagine/LVM|LVM]]
- [[Linux/Pagine/RAID software|RAID software]]
- [[Linux/Pagine/Swap|Swap]]
- [[Linux/Pagine/Quote disco|Quote disco]]
- [[Linux/Pagine/Controllo e riparazione filesystem|Controllo e riparazione filesystem]]

### Networking

- [[Linux/Pagine/Interfacce e indirizzi IP|Interfacce e indirizzi IP]]
- [[Linux/Pagine/Routing|Routing]]
- [[Linux/Pagine/DNS|DNS]]
- [[Linux/Pagine/Porte e socket|Porte e socket]]
- [[Linux/Pagine/SSH|SSH]]
- [[Linux/Pagine/Trasferimento file con scp e rsync|Trasferimento file con scp e rsync]]
- [[Linux/Pagine/NetworkManager|NetworkManager]]
- [[Linux/Pagine/Firewall con nftables|Firewall con nftables]]
- [[Linux/Pagine/Diagnostica con ip ss ping e traceroute|Diagnostica con ip, ss, ping e traceroute]]

## Percorso Avanzato

### Boot e kernel

- [[Linux/Pagine/Processo di boot|Processo di boot]]
- [[Linux/Pagine/UEFI e BIOS|UEFI e BIOS]]
- [[Linux/Pagine/Bootloader GRUB e systemd-boot|Bootloader GRUB e systemd-boot]]
- [[Linux/Pagine/Initramfs|Initramfs]]
- [[Linux/Pagine/Moduli del kernel|Moduli del kernel]]
- [[Linux/Pagine/Parametri del kernel e sysctl|Parametri del kernel e sysctl]]
- [[Linux/Pagine/Compilazione del kernel|Compilazione del kernel]]

### Sicurezza

- [[Linux/Pagine/Principio del minimo privilegio|Principio del minimo privilegio]]
- [[Linux/Pagine/Hardening del sistema|Hardening del sistema]]
- [[Linux/Pagine/AppArmor e SELinux|AppArmor e SELinux]]
- [[Linux/Pagine/Crittografia con LUKS|Crittografia con LUKS]]
- [[Linux/Pagine/GPG|GPG]]
- [[Linux/Pagine/PAM|PAM]]
- [[Linux/Pagine/Audit e log di sicurezza|Audit e log di sicurezza]]
- [[Linux/Pagine/Aggiornamenti di sicurezza|Aggiornamenti di sicurezza]]

### Hardware e desktop

- [[Linux/Pagine/Rilevamento hardware|Rilevamento hardware]]
- [[Linux/Pagine/Driver e firmware|Driver e firmware]]
- [[Linux/Pagine/udev|udev]]
- [[Linux/Pagine/D-Bus|D-Bus]]
- [[Linux/Pagine/Gestione energetica|Gestione energetica]]
- [[Linux/Pagine/Audio con PipeWire|Audio con PipeWire]]
- [[Linux/Pagine/Xorg e Wayland|Xorg e Wayland]]
- [[Linux/Pagine/Desktop environment e window manager|Desktop environment e window manager]]
- [[Linux/Pagine/Gestione dei font|Gestione dei font]]
- [[Linux/Pagine/Portali desktop e applicazioni predefinite|Portali desktop e applicazioni predefinite]]

### Virtualizzazione e isolamento

- Namespace
- Cgroups
- Container
- Docker e Podman
- KVM e QEMU
- Macchine virtuali con libvirt

## Applicazioni e Amministrazione

### Server

- Server SSH
- Web server
- Reverse proxy
- Condivisioni NFS e Samba
- Database su Linux
- Backup e ripristino
- Monitoraggio del sistema

### Automazione

- Bash scripting
- Script robusti
- Gestione degli errori negli script
- Variabili e configurazione
- Cron e timer systemd
- Ansible

## Distribuzioni

### Installazione

- [[Linux/Pagine/Installazione Arch Linux|Installazione Arch Linux]]
- Installazione Debian
- Installazione Fedora

### Differenze tra distribuzioni

- Package manager delle distribuzioni
- Rolling release e distribuzioni stabili
- Configurazioni specifiche delle distribuzioni
- Scelta della distribuzione

## Conoscenza Operativa

### Procedure

- Recuperare spazio su disco
- Riparare un sistema che non si avvia
- Recuperare l'accesso a un account
- Montare un disco
- Configurare una chiave SSH
- Creare un servizio systemd
- Eseguire un backup con rsync

### Checklist

- Checklist installazione Linux
- Checklist sicurezza Linux
- Checklist manutenzione server
- Checklist backup

### Troubleshooting

- Diagnostica boot
- Diagnostica rete
- Diagnostica DNS
- Diagnostica permessi
- Diagnostica systemd
- Diagnostica spazio disco
